...
class PostgresPersistenceEngine extends AbstractPersistenceEngine {
  ...
  async events (persistenceKey, offset = 0, limit = null, tags) {
    ...
    const result = (await this.db).any(query, args).then(results => results.map(PostgresPersistenceEngine.mapDbModelToDomainModel));
